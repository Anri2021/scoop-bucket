              ...
              "node"{
              	...
              	if(-not $entry){
              	  if(Test-Path "package.json"){
              	    ...
              	    if($pkgJson.bin){ $entry = if($pkgJson.bin -is [string]){ $pkgJson.bin } else { ($pkgJson.bin.PSObject.Properties | Select-Object -First 1).Value } }
              	    elseif($pkgJson.main){ $entry = $pkgJson.main }
              	  }
              	...
              	try{
              	  if(Test-Path "pnpm-lock.yaml"){
              	    Cmd "pnpm" @("install","--prod","--frozen-lockfile","--config.node-linker=hoisted")
              	  }else{
              	    Cmd "npm" @("install","--omit=dev","--ignore-scripts","--no-audit","--no-fund")
              	  }
              	}finally{Pop-Location}
